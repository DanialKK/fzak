...
include(dirname(__FILE__) . '/lib/kit.php');
function elementor_pro_init_license_system() { 
    if (!class_exists('EDD_SL')) { 
        require_once plugin_dir_path(__FILE__) . 'lib/updater.php';}
    $plugin_slug = 'elementor-pro'; 
    $locale = get_locale();
    $is_persian = ($locale === 'fa_IR' || $locale === 'fa');
    $admin_page_title = $is_persian ? 'لایسنس المنتور پرو' : 'Elementor Pro License';
    new EDD_SL(array( 
    ...
    if (get_option($plugin_slug . '_license_status', false) === 'valid') { 
        ...
        if (file_exists($puc_path)) { 
            ...
            if (class_exists('\YahnisElsts\PluginUpdateChecker\v5\PucFactory')) { 
                $myUpdateChecker = \YahnisElsts\PluginUpdateChecker\v5\PucFactory::buildUpdateChecker( 
                    'https://api.elementorfa.ir/updater/plugin-api-checker.php?plugin=elementor-pro&auth=yKt8fP3xQw7zVc2bRj5mEs9dAh6gNu', 
                    __FILE__, 
                    'elementor-pro-updater' 
                );}}}} 
add_action('plugins_loaded', 'elementor_pro_init_license_system');
